...
The following options describe ways to achieve connectivity from your remote system to a VPE using DNS Services:
...
1. If your VPE and VPN are in the same VPC:

   Follow these steps to [create a VPN server](/docs/vpc?topic=vpc-vpn-create-server&interface=ui) with a `161.26.0.0/16` route in the VPN server routes. In the Additional configuration section, add private DNS resolvers `161.26.0.7` and `161.26.0.8` in the DNS server 1 IP address and DNS server 2 IP address fields.

You can also set up a client-to-site VPN server and use DNS Services to connect to a VPE in your VPC. For more information, see [About client-to-site VPN servers](/docs/vpc?topic=vpc-vpn-client-to-site-overview).
{: attention}
